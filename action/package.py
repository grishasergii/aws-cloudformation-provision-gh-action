...
def package(stack_name, path, main_template, branch, artifacts_bucket, cfn_bucket):
    ...
    for template_file in os.listdir(path):
        if template_file.endswith(".cfn"):
            ...
            ExternalCmd.run(
                ...
                f"--s3-prefix {stack_name}/{branch} "
                f"--output-template-file {out_path}"
            )

            click.echo(f"Uploading {out_path} to {cfn_bucket}")
            ExternalCmd.run(
                f"aws s3 cp {out_path} s3://{cfn_bucket}/{stack_name}/{branch}/{template_file}"
            )
